...
public:
	...
	GLint Model::TextureFromFile(const char* path, string directory)
	{
		...
		unsigned char* image = SOIL_load_image(filename.c_str(), &width, &height, &canaux, SOIL_LOAD_RGB);
		// Assign texture to ID
		glBindTexture(GL_TEXTURE_2D, textureID);
		glTexImage2D(GL_TEXTURE_2D, 0, canaux, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
